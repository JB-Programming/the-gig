...
const Teamschlüssel = (selectedNode) => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:8000/api/get_teamschluessel_data/', {
                year: selectedYear,
            ...
            if (response.data) {
                ...
                let new_data = response.data.map((row, index) => ({
                    ...row,
                    provisionssatz: formatPercentage(response.data[index].provisionssatz || '0'),
                    }));
    ...
    const saveData = async () => {
        ...
        try {
            const data = monthlyData.map(row => ({
                ...row,
                provisionssatz: parseFloat(stripFormatting(row.provisionssatz))
              }));
            const response = await axios.post('http://localhost:8000/api/save_teamschluessel/', {
                data: data,
                year: selectedYear,
                team: selectedNode.selectedNode.name
            }, {
    ...
    const formatPercentage = (num) => {
        ...
        const numWithComma = num.toString().replace('.', ',');
                    ...
                    <TableBody>
                        ...
                        <TableRow sx={{ backgroundColor: '#e9ecef', fontWeight: 'bold' }}>
                            ...
                            <TableCell sx={{ textAlign: 'right' }}>
                                {formatPercentage(monthlyData.reduce((sum, row) => {
                                    return sum + parseFloat(stripFormatting(row.provisionssatz));
                                }, 0).toString())}
                            </TableCell>
                        </TableRow>
                    </TableBody>
